...
d3.csv("../data/crime_daily_counts.csv").then(data => {
  ...
  function openCrimeTypeModal(date, filteredData) {
    ...
    shiftSvg.selectAll(".bar")
      ...
      .append("rect")
      .attr("class", "bar")
      .attr("x", (d, i) => xScaleShift(shiftLabels[i]))
      .attr("y", d => yScaleShift(d))
    ...
    wardSvg.selectAll(".bar")
      ...
      .attr("class", "bar")
      .attr("x", xScaleWard(0))
      .attr("y", (d, i) => yScaleWard(wardLabels[i]))
  ...
  // Function to draw the pie chart and bar chart in the detailed view
  function openPieChartModal(date, filteredData) {
    ...
    offenseKeySvg.selectAll(".bar")
      ...
      .append("rect")
      .attr("class", "bar")
      .attr("x", (d, i) => xScaleOffenseKey(offenseKeyLabels[i]))
      .attr("y", d => yScaleOffenseKey(d))
  ...
  function drawHeatCalendar(filteredData) {
    ...
    svg.selectAll(".cell")
      ...
      .on("mouseout", () => {
        d3.select("#timeline-tooltip").style("visibility", "hidden").style("opacity", 0);
        // Clear highlights in the bar chart
        highlightBarChart([]);
      })
      .on("click", (event, d) => {
        // Open modal and draw pie chart
